...
#ryu-manager --ofp-listen-host=127.0.0.1 --ofp-tcp-listen-port=6653 final_controller.py
...
class MyController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.timeout = 5
        self.packet_info = {}  #Dictionaries to save packet information using flow_key as identifier
    ...
    def handle_packet(self,parser,msg,in_port, match, actions, datapath, flow_key):
        ...
        # print(f'{flow_key[0]} to {flow_key[1]}: No. {self.packet_counters[flow_key]}') #printing mac addresses of flow with flow_counter
        ...
        if self.packet_counters[flow_key] == 10:#change this for threshold
            ...
            # TODO: when features are calculated , these will be passed to our trained model(RF/DT/XGBoost) and a flow will be added accordingly(Benign/Attack)


            self.add_flow(datapath, 1, match, actions, hard=20)# hard timeout for flow expiry
            print(f"Flow entry added for src: {flow_key[0]} and dst: {flow_key[1]}.")
            print()
    ...
    # def has_existing_flow(self, datapath, match):
    #     # Check if there is an existing flow entry with the same match conditions
    #     existing_flows = self.installed_flows.get(datapath.id, [])
    #     return match in existing_flows
    #         # packet_time = time.time()
    #         # print(f"Packet received at time {packet_time}")
    #         # print()
        
    # def update_existing_flows(self, datapath, match):
    #     # Update the existing flows with the new match conditions
    #     existing_flows = self.installed_flows.get(datapath.id, [])
    #     existing_flows.append(match)
    #     self.installed_flows[datapath.id] = existing_flows
